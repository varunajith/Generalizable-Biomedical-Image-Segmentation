# -*- coding: utf-8 -*-
"""
Created on Mon Mar 17 16:08:26 2025
...
def apply_watershed(prediction):
    """
    Applies the Watershed Transform to separate touching objects in a binary prediction.
    
    Args:
        prediction (numpy.ndarray): Binary prediction mask (0 = background, 1 = foreground).
    
    Returns:
        numpy.ndarray: Labeled segmentation mask where each object has a unique ID.
    """
    # Compute the distance transform of the binary prediction
    distance = ndi.distance_transform_edt(prediction)
    
    # Find local maxima (peaks) in the distance transform
    coords = peak_local_max(distance, footprint=np.ones((3, 3)), labels=prediction)
    ...
    labels = watershed(-distance, markers, mask=prediction)
    
    return labels


def evaluate_segmentation(net, valid_iterator, device,criterion,n_valid_examples,is_avg_prec=True,prec_thresholds=[0.5,0.7,0.9],output_dir=None):
        ...
        for batch_idx,batch in enumerate(valid_iterator):
            ...
            with torch.no_grad():
                ...
                if is_avg_prec:
                    ...
                    assert mask_pred_copy.shape[0] == true_masks_copy.shape[0], "Mismatch in number of true/pred masks in batch"
                    
                    
                    ######################################watershed
                    #for i in range(true_masks_copy.shape[0]):
                    # Apply Watershed Transform to predicted mask
                        #pred_binary = mask_pred_copy[i, :, :] > 0  # Convert to binary mask
                        #pred_watershed = apply_watershed(pred_binary)
    
                    # Remove small objects (optional)
                        #pred_watershed, _ = ndi.label(remove_small_objects(pred_watershed > 0, min_size=15, connectivity=1))
                    #################################################
                    ...
                    for i in range(true_masks_copy.shape[0]):
                        ...
                        if output_dir:
                            ...
                            #pred_img = pred_watershed * 255  # Scale to [0, 255] Watershed
                    ...
                    #######watershed
                    #pred_list.append(pred_watershed)
    ...
    if is_avg_prec:
        #print(f"Number of true masks: {len(mask_list)}")
        #print(f"Number of predicted masks: {len(pred_list)}")
...
def average_precision_recall_fscore(masks_true, masks_pred, threshold=[0.5, 0.75, 0.9]):
    ...
    assert len(masks_true) == len(masks_pred), "Mismatch in number of true/pred masks"
    # Correct instance counting
    #n_true = np.array([len(np.unique(m)) - 1 for m in masks_true])
    ...
    with tqdm(total=len(masks_true), desc='Metrics measurement', unit='img') as pbar:
        for n in range(len(masks_true)):
            ...
            #fp[n] = max(n_pred[n] - tp[n],0)
            #fn[n] = max(n_true[n] - tp[n],0)
            fp[n] = n_pred[n] - tp[n]
            fn[n] = n_true[n] - tp[n]
