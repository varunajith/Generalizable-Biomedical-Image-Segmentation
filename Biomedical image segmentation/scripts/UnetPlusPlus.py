""" Full assembly of the parts to form the complete network (U-Net++) """
...
class UNetPlusPlus(nn.Module):
    ...
    def forward(self, x):
        ...
        for j in range(1, 5):  # j: 1 to 4
            ...
            for i in range(0, 5 - j):
                # Get deeper node (i+1, j-1)
                deeper_node = decoder_outputs[j-1][i+1]
                # Get previous node (i, j-1)
        ...
        # The final output is the last node in the top row (X0,4)
